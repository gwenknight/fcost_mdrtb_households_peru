...
for(i in 1:nfits){
  ...
  if(file.exists(paste("bi_fp1_summary_fit_",i,".csv",sep=""))){
    st <- read.csv(paste("bi_fp1_summary_fit_",i,".csv",sep=""))[,-1]
    all_fits<-rbind(all_fits,t(st))
  }
  if(file.exists(paste("bi_fp1_kp_",i,".csv",sep=""))){
    kp <- read.csv(paste("bi_fp1_kp_",i,".csv",sep=""))[,-1]
...
all_fitsm <- melt(all_fits[1:100,c("TBIr_s", "TBIr_r", "TBIs_s", "TBIs_r")],id.vars=c()) # Perhaps only plot first 100? ****
# data points
...
ggsave("bi_Fit_model1.pdf",width=12,height=8)
...
ggsave("bi_Fit_model1_kp_plain.pdf",width=12,height=8)
...
g<-g+scale_x_continuous("Follow up Time (days)") + scale_y_continuous("Probability of remaining Free of Tuberculosis",lim=c(0.91,1),breaks=seq(0.91,1,0.01))
#g<-g+ theme(legend.key.size = unit(1, "cm"))
ggsave("bi_Fit_model1_kp.pdf",width=12,height=8)
...
ggsave("bi_Fit_model1_kp_round.pdf",width=12,height=8)
...
fits1<-c()
for(i in 1:nfits){
  if(file.exists(paste("bi_fp1_fits_",nsteps,"_",i,".csv",sep=""))){
  ffn <- read.csv(paste("bi_fp1_fits_",nsteps,"_",i,".csv",sep=""))[-1,-1] # remove first row as that's the basic start guess
