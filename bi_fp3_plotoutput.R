...
for(i in 1:nfits){
  ...
  if(file.exists(paste("bi_fp3_summary_fit_",i,".csv",sep=""))){
    st <- read.csv(paste("bi_fp3_summary_fit_",i,".csv",sep=""))[,-1]
    all_fits<-rbind(all_fits,t(st))
  }
  if(file.exists(paste("bi_fp3_kp_",i,".csv",sep=""))){
    kp <- read.csv(paste("bi_fp3_kp_",i,".csv",sep=""))[,-1]
...
ggsave("bi_Fit_model3.pdf",width=12,height=8)
...
ggsave("bi_Fit_model3_kp_plain.pdf",width=12,height=8)
...
ggsave("bi_Fit_model3_kp.pdf",width=12,height=8)
...
g<-g+scale_colour_manual("",breaks = c("1","2"),values=c("red","green"),labels = c("Contacts of Drug\nResistant Tuberculosis","Contacts of Drug\nSusceptible Tuberculosis"))
g<-g+scale_x_continuous("Follow up Time (days)") + scale_y_continuous("Probability of remaining Free of Tuberculosis",lim=c(0.91,1))
ggsave("bi_Fit_model3_kp_round.pdf",width=12,height=8)
...
fits1<-c()
for(i in 1:nfits){
  if(file.exists(paste("bi_fp3_fits_",nsteps,"_",i,".csv",sep=""))){
  ffn <- read.csv(paste("bi_fp3_fits_",nsteps,"_",i,".csv",sep=""))[-1,-1] # remove first row as that's the basic start guess
